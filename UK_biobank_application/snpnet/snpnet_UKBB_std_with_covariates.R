...
predict_snpnet_hk <- function(fit, new_genotype_file, new_phenotype_file, phenotype, subset=NULL){
  ...
  chosen_SNPs <- cbind(str_split(names(final_betas)[-(1:(length(fit$configs[['covariates']])))],"_",simplify = T),
                       final_betas[-(1:(length(fit$configs[['covariates']])))])
  ...
  pred <- matrix(fit$a0[[idx]] + phe_PRS$PRS + if(!is.null(fit$configs[['covariates']])){as.matrix(phe_PRS%>%select(fit$configs[['covariates']])) %*% final_betas[fit$configs[['covariates']]]}else{0},ncol=1)
...
covariates <- c("sex","age_at_2015",paste0("PC",1:10))

configs <- list(plink2.path = "plink2", # plink2 path
                zstdcat.path = "zstdcat", # zstdcat path
                results.dir = paste0("non_imputed_data_snpnet_std_cov_",phenotype),
                save = TRUE,
                prevIter = 0,
                missing.rate = 0.1,
                MAF.thresh = 0.001,
...
nmb_snps <- sum(fit_snpnet$beta[[idx]]!=0)-length(fit_snpnet$configs[['covariates']])
...
MSEP_test_full_model=pred_test_snpnet$metric

cor_squared_test_full_model=cor(data[data$train_test=="test",]%>%pull(all_of(phenotype)),pred_test_snpnet$prediction)^2
...
save.image(file=paste0("results/",phenotype,"_snpnet_std_covariates.RData"))
