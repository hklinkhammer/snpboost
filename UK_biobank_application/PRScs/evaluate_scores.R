...
scores <- fread(paste0("Scores/",phenotype,".sscore")) %>% rename(IID='#IID')

phenotypes <- fread("phenotypes_unrelated.phe")

df <- left_join(phenotypes,scores)

print(paste("correlation", phenotype,"and SCORE1_AVG:", round(cor(df[[phenotype]][df$train_test=="test"],df$SCORE1_AVG[df$train_test=="test"])^2,4)))
